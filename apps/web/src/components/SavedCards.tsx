...
export default function SavedCards() {
  ...
  async function loadMore() {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/saved?limit=${BATCH_SIZE}&offset=${offset()}`)
      ...
      const existingIds = new Set(cards().map((c) => c.card.id))
      const newCards = batch.filter((c) => !existingIds.has(c.card.id))
  ...
  return (
    <div class="flex flex-col gap-10">
      <h1 class="font-display text-xl md:text-3xl text-ed-on-surface">Saved</h1>
      ...
      <For each={cards()}>
        {(item) => {
          ...
          // Card is removed when neither liked nor bookmarked
          function checkRemoved(action: string, active: boolean) {
            const currentActions = new Set(item.actions)
            if (active) currentActions.add(action)
            else currentActions.delete(action)
            item.actions = [...currentActions]
            if (!currentActions.has('like') && !currentActions.has('bookmark')) {
              setRemoved(true)
            }
          }
              ...
              <div>
                ...
                <div class="mb-2 flex items-baseline justify-between gap-2">
                  ...
                  <a
                    ...
                    class="truncate font-body text-[0.65rem] tracking-wide text-ed-on-surface-muted hover:text-ed-primary transition-colors"
                  >
                    {item.document.title}
                  </a>
                </div>
                ...
                <div class={`rounded p-6 ${item.card.cardType === 'flashcard' ? 'bg-ed-surface-dim' : 'bg-ed-surface-high'}`}>
                  ...
                  <div class="flex items-center gap-1 mt-3 pt-3 border-t border-ed-outline-dim">
                    <ActionButton
                      ...
                      title="Like"
                      onToggle={(active) => checkRemoved('like', active)}
                    />
                    <ActionButton
                      cardId={item.card.id}
                      action="bookmark"
                      ...
                      title="Bookmark"
                      onToggle={(active) => checkRemoved('bookmark', active)}
                    />
                    <ActionButton
                      cardId={item.card.id}
                      action="dismiss"
                      ...
                      title="Dismiss"
                      onToggle={(active) => { if (active) setRemoved(true) }}
                    />
                  </div>
                </div>
              </div>
      ...
      <Show when={cards().length === 0 && initialLoaded() && !error()}>
        <div class="flex flex-col items-center gap-4 py-20 text-center">
          <p class="font-display text-lg text-ed-on-surface-dim">No saved cards yet.</p>
          <p class="font-body text-sm text-ed-on-surface-muted">
            Like or bookmark cards from your feed to see them here.
          </p>
          <a
            href="/feed"
