...
function renderMarkdown(html: string): string {
  ...
  const codeLangs =
    "(?:python|javascript|typescript|java|rust|go|c|cpp|bash|sh|r|sql|ruby|swift|kotlin|scala|html|css)";
  html = html.replace(
    new RegExp(`(?:^|\\n\\n)${codeLangs}\\n([\\s\\S]+?)(?=\\n\\n[A-Z]|$)`, "g"),
    (_match, code) => {
  ...
  html = html.replace(
    /`([^`]+)`/g,
    '<code class="rounded bg-ctp-surface1 px-1 py-0.5 text-[0.85em] font-mono">$1</code>',
  );
  ...
  const rendered = blocks.map((block) => {
    ...
    if (trimmed.startsWith("<div") || trimmed.startsWith("<pre"))
      return trimmed;
    ...
    if (isOrdered) {
      const items = lines
        .map((l) => `<li>${l.trimStart().replace(/^\d+\.\s/, "")}</li>`)
        .join("");
